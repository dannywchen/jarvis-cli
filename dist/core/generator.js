 ...
 */
export async function generateCourse(doc, pace = 'standard', config) {
    ...
    if (pace === 'accelerated')
        targetNodeCount = Math.min(4, Math.max(3, sectionCount));
    else if (pace === 'deep')
        targetNodeCount = Math.min(12, Math.max(8, sectionCount * 2));
    else
        targetNodeCount = Math.min(8, Math.max(5, sectionCount));
    const nodes = [];
    ...
    for (let i = 0; i < targetNodeCount; i++) {
        ...
        const lessons = [
            generateBiteSizedLesson(`lesson_${i}_1`, `${nodeTitle.replace(/^[⚔️\s]+/, '')} Fundamentals`, contentSnippet, nodeFacts, keywords, isBoss),
        ];
        if (pace === 'deep' && !isBoss) {
            lessons.push(generateBiteSizedLesson(`lesson_${i}_2`, `Practical Application & Deep Dive`, contentSnippet.slice(300), nodeFacts, keywords, false));
        }
        nodes.push({
            id: `node_${i + 1}`,
...
function generateBiteSizedLesson(id, title, contentSnippet, facts, keywords, isBoss) {
    ...
    if (facts.length >= 2) {
        const pairs = facts.slice(0, 3).map((f) => ({
            term: f.term,
