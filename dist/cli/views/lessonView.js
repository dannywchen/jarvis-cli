...
export async function runLesson(lesson, node, course, profile) {
    ...
    const digestBox = boxen(chalk.hex('#CBD5E1').bold('DIGEST & MENTAL MODEL\n\n') +
    ...
    for (let idx = 0; idx < totalQuestions; idx++) {
        ...
        if (q.type === 'multiple-choice' || q.type === 'scenario') {
            ...
            if (choice === 'ask_hint') {
                console.log(boxen(chalk.hex('#94A3B8').bold('Coaching Hint:\n') +
                    chalk.hex('#E2E8F0')(q.hint || `Focus on how this relates to ${node.title}.\nRemember: ${lesson.keyTakeaway}`), { padding: 1, borderColor: 'gray', borderStyle: 'round' }));
            ...
            if (choice === 'custom_ai') {
                ...
                if (p.isCancel(freeform) || !freeform)
                    return { success: false, xpEarned: totalXp };
        ...
        if (isCorrect) {
            ...
            if (profile.hearts <= 0 && !profile.zenMode) {
                console.log(boxen(chalk.hex('#F87171').bold('OUT OF HEALTH SHIELDS\n\n') +
                    chalk.hex('#CBD5E1')('You have depleted all active HP.\nRun practice mode (/practice) to restore shields or switch to Zen Mode.'), { padding: 1, borderColor: 'gray', borderStyle: 'round' }));
    ...
    console.log(boxen(chalk.hex('#F8FAFC').bold('LESSON COMPLETE\n\n') +
        ...
        chalk.hex('#94A3B8')(`Streak: ${profile.streak} days active`), { padding: 1, borderColor: 'gray', borderStyle: 'round' }));
