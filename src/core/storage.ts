...
function getLegacyStorageDirectory(): string | null {
  if (process.env.JARVIS_CLI_DATA_DIR || process.env.DUOCODE_DATA_DIR) return null;
  return path.join(os.homedir(), LEGACY_DUO_DIR_NAME);
}

function getStoragePaths(dataDir = getStorageDirectory()): StoragePaths {
  return {
    dataDir,
...
export function sanitizeLoadedProfile(parsed: Partial<UserProfile>): UserProfile {
...
export async function loadUserProfile(): Promise<UserProfile> {
  ...
  const candidates = [profileFile, legacyDirectory ? path.join(legacyDirectory, 'profile.json') : null].filter(
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (candidate !== profileFile || hadLegacyArtificialXp) await saveUserProfile(profile);
      return profile;
    } catch {
...
export async function listSavedCourses(): Promise<Course[]> {
  ...
  const { coursesDir } = getStoragePaths();
  const legacyDirectory = getLegacyStorageDirectory();
  const courseDirectories = [coursesDir, legacyDirectory ? path.join(legacyDirectory, 'courses') : null].filter(
...
export async function loadReviewItems(): Promise<SpacedReviewItem[]> {
  ...
  const legacyDirectory = getLegacyStorageDirectory();
  const candidates = [reviewsFile, legacyDirectory ? path.join(legacyDirectory, 'reviews.json') : null].filter(
    (candidate): candidate is string => Boolean(candidate),
  );
  for (const candidate of candidates) {
    try {
...
function queueHistoryWrite<T>(operation: () => Promise<T>): Promise<T> {
...
function normalizeRole(value: unknown): ChatMessageRole | null {
  const role = asNonEmptyString(value)?.toLowerCase();
  if (role === 'user' || role === 'human' || role === 'prompt') return 'user';
  if (role === 'assistant' || role === 'agent' || role === 'bot' || role === 'ai') return 'assistant';
...
function normalizeMessage(value: unknown, threadId: string, index: number, fallbackTime: string): ChatMessage | null {
...
function findMessageArray(value: Record<string, unknown>): unknown[] {
...
function normalizeThread(value: unknown, index: number): RecentThread | null {
  if (!isRecord(value)) return null;
...
function findThreadArray(value: unknown): unknown[] {
  if (Array.isArray(value)) return value;
  if (!isRecord(value)) return [];
  for (const key of ['threads', 'recentThreads', 'sessions', 'conversations', 'history']) {
    if (Array.isArray(value[key])) return value[key];
  }
  if (value.id || value.threadId || value.sessionId || value.messages || value.transcript) return [value];
  return [];
}

function sortAndLimitThreads(threads: RecentThread[]): RecentThread[] {
  const deduped = new Map<string, RecentThread>();
  for (const thread of threads) deduped.set(thread.id, thread);
  return [...deduped.values()]
    .sort((left, right) => new Date(right.updatedAt).getTime() - new Date(left.updatedAt).getTime())
    .slice(0, MAX_RECENT_THREADS);
}

function normalizeThreads(value: unknown): RecentThread[] {
  return sortAndLimitThreads(
    findThreadArray(value)
      .map((thread, index) => normalizeThread(thread, index))
      .filter((thread): thread is RecentThread => thread !== null)
  );
}

async function readJson(filePath: string): Promise<ReadJsonResult> {
  try {
    const raw = await fs.readFile(filePath, 'utf-8');
    try {
      return { exists: true, valid: true, value: JSON.parse(raw) as unknown };
    } catch {
...
async function loadRecentThreadsInternal(): Promise<RecentThread[]> {
  ...
  for (const directory of historySearchDirectories()) {
    for (const fileName of LEGACY_THREAD_FILE_NAMES) {
      ...
      if (!legacy.exists || !legacy.valid) continue;
      const migrated = normalizeThreads(legacy.value);
      if (!migrated.length) continue;
      await writeJsonAtomically(paths.recentThreadsFile, { version: HISTORY_VERSION, threads: migrated } satisfies RecentThreadHistory);
      return migrated;
    }
...
function threadForPersistence(thread: RecentThread): RecentThread {
  const normalized = normalizeThread(thread, 0);
  if (!normalized) throw new Error('Cannot persist an invalid recent thread.');
  return normalized;
}

export async function loadRecentThreads(): Promise<RecentThread[]> {
  await ensureStorageDirectories();
  const threads = await loadRecentThreadsInternal();
  return threads.map(cloneThread);
}

export async function saveRecentThreads(threads: RecentThread[]): Promise<void> {
  await ensureStorageDirectories();
  await queueHistoryWrite(async () => {
    ...
    await writeJsonAtomically(recentThreadsFile, { version: HISTORY_VERSION, threads: normalized } satisfies RecentThreadHistory);
  });
}

export async function createRecentThread(options: CreateRecentThreadOptions = {}): Promise<RecentThread> {
  await ensureStorageDirectories();
  return queueHistoryWrite(async () => {
    ...
    const { recentThreadsFile } = getStoragePaths();
    await writeJsonAtomically(recentThreadsFile, { version: HISTORY_VERSION, threads: next } satisfies RecentThreadHistory);
    return cloneThread(thread);
  });
}

export async function loadRecentThread(threadId: string): Promise<RecentThread | null> {
  const thread = (await loadRecentThreads()).find((item) => item.id === threadId);
  return thread ? cloneThread(thread) : null;
}

export async function saveRecentThread(thread: RecentThread): Promise<RecentThread> {
  await ensureStorageDirectories();
  return queueHistoryWrite(async () => {
    const threads = await loadRecentThreadsInternal();
...
export async function appendRecentThreadMessage(
  threadId: string,
  ...
  return queueHistoryWrite(async () => {
    ...
    if (!nextMessage.text) return cloneThread(existing);

    const messages = [...existing.messages, nextMessage].slice(-MAX_MESSAGES_PER_THREAD);
    const nextThread: RecentThread = {
      ...existing,
...
export function summarizeRecentThreads(threads: RecentThread[], limit = MAX_RECENT_THREADS): RecentThreadSummary[] {
