...
function comparableLearningText(text: string): string {
...
function isNearDuplicate(candidate: string, previous: string[]): boolean {
  ...
  if (!candidateWords.size) return false;
...
function studyContext(topic: string, activeCourse?: Course | null): string {
  if (!activeCourse) return `Requested topic: ${topic}`;
...
function previousLearningPrompts(profile: UserProfile | undefined, kind: GeneratedLearningHistoryEntry['kind'], topic: string): string[] {
  if (!profile?.generatedLearningHistory?.length) return [];
...
export function rememberGeneratedLearning(
  profile: UserProfile,
  kind: GeneratedLearningHistoryEntry['kind'],
  topic: string,
  prompts: string[]
): void {
  if (!prompts.length) return;
  const entry: GeneratedLearningHistoryEntry = {
    kind,
    topic: topic.trim(),
...
function normalizeGeneratedQuestion(raw: any, index: number, topic: string): Question | null {
  const type = raw?.type === 'scenario' ? 'scenario' : raw?.type;
  const prompt = typeof raw?.prompt === 'string' ? raw.prompt.trim() : '';
  if (!prompt || prompt.length < 12 || !['multiple-choice', 'scenario', 'cloze', 'open-ended'].includes(type)) return null;
  ...
  if (type === 'cloze') {
    if (typeof raw.clozeAnswer !== 'string' || !raw.clozeAnswer.trim()) return null;
  ...
  if (options.length < 4 || correctIndex < 0 || correctIndex >= options.length) return null;
  base.options = options.slice(0, 4);
  base.correctIndex = correctIndex;
  return base;
}

function courseQuestionFallback(
  topic: string,
...
function isRelevantToRequest(question: Question, topic: string, activeCourse?: Course | null): boolean {
...
async function generateWithActiveAgent(
  profile: UserProfile,
  prompt: string,
  options: LearningGenerationOptions
): Promise<string | null> {
  if (options.forceOffline || process.env.JARVIS_OFFLINE === '1' || process.env.JARVIS_TEST_OFFLINE === '1') return null;
  const creds = resolveActiveCredentials(profile);
  if (!creds.hasAuth) return null;
  ...
  const timeoutPromise = new Promise<{ text: string; error?: string }>((resolve) =>
    setTimeout(() => resolve({ text: '', error: 'Learning generation timed out' }), options.timeoutMs || 10000)
  );
...
export async function generateOnTheFlyQuiz(
  ...
  const attempt = (profile?.generatedLearningHistory || []).filter(
    (entry) => entry.kind === 'quiz' && comparableLearningText(entry.topic) === comparableLearningText(cleanTopic),
  ).length + 1;
  ...
  try {
    ...
    if (text) {
      ...
      const generated = (Array.isArray(parsed?.questions) ? parsed.questions : [])
        .map((raw: any, index: number) => normalizeGeneratedQuestion(raw, index, cleanTopic))
        .filter((question: Question | null): question is Question => Boolean(question))
        .filter((question: Question) => isRelevantToRequest(question, cleanTopic, activeCourse))
        .filter((question: Question) => {
          const key = comparableLearningText(question.prompt);
          if (seen.has(key) || isNearDuplicate(question.prompt, previous)) return false;
          seen.add(key);
          return true;
        })
        .slice(0, 4);
      if (generated.length >= 2 && generated.some((question: Question) => question.type === 'open-ended')) return generated;
    }
  } catch {
...
export async function generateOnTheFlyFlashcards(
  topic: string,
  activeCourse?: Course | null,
  profile?: UserProfile,
  options: LearningGenerationOptions = {}
  ...
  try {
    ...
    if (text) {
      ...
      const generated = (Array.isArray(parsed?.flashcards) ? parsed.flashcards : [])
        ...
        .filter((card: GeneratedFlashcard) => card.front.length >= 8 && card.back.length >= 12)
        .filter((card: GeneratedFlashcard) => {
          const key = comparableLearningText(card.front);
          if (seen.has(key) || isNearDuplicate(card.front, previous)) return false;
          seen.add(key);
          return true;
        })
        .slice(0, 6);
