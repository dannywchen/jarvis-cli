...
export function normalizeModelName(rawModel?: string): string {
  ...
  if (lower.includes('gemini-2.5-flash') || lower.includes('gemini-2.0-flash')) return 'Gemini 2.5 Flash';
  if (lower.includes('gemini-2.5-pro') || lower.includes('gemini-3-pro')) return 'Gemini 3 Pro';
  if (lower.includes('gpt-5.6') || lower.includes('luna')) return 'GPT-5.6 Luna';
  if (lower.includes('gpt-4o')) return 'GPT-4o';
  if (lower.includes('claude-3-5-sonnet') || lower.includes('claude-3.5-sonnet')) return 'Sonnet 3.5';
  if (lower.includes('claude-3-7-sonnet') || lower.includes('claude-3.7-sonnet')) return 'Sonnet 3.7';
  if (lower.includes('sonnet-4.5') || lower.includes('claude-4.5-sonnet')) return 'Sonnet 4.5';
  return rawModel;
}

export function formatCwd(dir?: string): string {
 ...
 * ╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┴┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈╯
 */
export function buildClaudeCodeBox(context: ClaudeBoxContext = {}, termWidth = 88): string[] {
  ...
  const center = (text: string, width: number): string => {
    ...
    if (raw.length >= width) return text.slice(0, width);
    const leftPad = Math.floor((width - raw.length) / 2);
    const rightPad = width - raw.length - leftPad;
    return ' '.repeat(leftPad) + text + ' '.repeat(rightPad);
  };

  const padRightAnsi = (styledText: string, width: number): string => {
    const rawLen = stripAnsi(styledText).length;
    if (rawLen >= width) return styledText;
...
export function renderBanner(context?: Partial<ClaudeBoxContext>): void {
...
export function renderJarvisChatBox(
  currentInput: string,
  _profile?: UserProfile,
  _activeCourse?: Course | null
): void {
...
export function renderStatusRibbon(profile: UserProfile, activeCourse?: Course | null): void {
