...
export async function runAiTutorSession(profile: UserProfile, activeCourse?: Course | null): Promise<void> {
  console.clear();
  ...
  console.log(
    boxen(
      chalk.hex('#F8FAFC').bold('Agent Dialogue Active\n\n') +
        ...
        chalk.hex('#475569')('Type "exit" or "done" to return to the hub.'),
  ...
  while (active) {
    ...
    if (p.isCancel(userInput) || !userInput || ['exit', 'quit', 'done'].includes((userInput as string).trim().toLowerCase())) {
      active = false;
      break;
    }

    const query = (userInput as string).trim();
