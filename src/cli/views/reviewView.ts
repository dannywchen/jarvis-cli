...
export async function runPracticeSession(profile: UserProfile, activeCourse?: Course | null): Promise<void> {
  ...
  if (dueReviews.length === 0 && (!activeCourse || activeCourse.nodes.length === 0)) {
    console.log(
      boxen(
        chalk.hex('#CBD5E1')('No overdue reviews or active nodes to practice currently.\n') +
          chalk.hex('#64748B')('Complete roadmap lessons to automatically register concepts into the SM-2 review queue.'),
  ...
  if (targetItem && targetQuestion) {
    ...
    if (targetQuestion.type === 'flashcard') {
      console.log(
        boxen(
          chalk.hex('#38BDF8').bold('REVIEW FLASHCARD FRONT\n\n') +
      ...
      if (p.isCancel(reveal)) return;

      console.log(
        boxen(
          chalk.hex('#10B981').bold('FLASHCARD BACK (EXPLANATION)\n\n') +
            chalk.hex('#F8FAFC')(targetQuestion.flashcardBack || targetQuestion.explanation),
      ...
      if (p.isCancel(confidence)) return;
      ...
      console.log(
        boxen(
          chalk.hex('#38BDF8').bold('REVIEW SYNTHESIS QUESTION\n\n') +
      ...
      const evalResult = await evaluateOpenEndedAnswer(targetQuestion, (input as string).trim(), profile);
    ...
    if (question) {
      if (question.type === 'flashcard') {
        console.log(
          boxen(
            chalk.hex('#38BDF8').bold('PRACTICE FLASHCARD FRONT\n\n') +
        ...
        console.log(
          boxen(
            chalk.hex('#10B981').bold('FLASHCARD BACK\n\n') +
              chalk.hex('#F8FAFC')(question.flashcardBack || question.explanation),
            { padding: 1, borderColor: 'green', borderStyle: 'round' }
          )
        );
        ...
        console.log(
          boxen(
            chalk.hex('#38BDF8').bold('PRACTICE SYNTHESIS QUESTION\n\n') +
        ...
        const spinner = p.spinner();
        spinner.start('✦ Evaluating answer with AI...');
        const evalResult = await evaluateOpenEndedAnswer(question, (input as string).trim(), profile);
