...
import { runConfigMenu } from './views/configView.js';
...
import { Course, Pace, SkillNode, Lesson } from '../types/index.js';
import { playChime } from './effects.js';

export async function startDuoCodeRepl(): Promise<void> {
  ...
  while (running) {
    ...
    if (lower.startsWith('/load') || lower.startsWith('load')) {
      ...
      if (targetFile) {
        ...
        if (newCourse) activeCourse = newCourse;
      }
      continue;
    }
